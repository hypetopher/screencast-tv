...
class PlaybackFragment : VideoSupportFragment() {
    ...
    private val castListener: (CastEvent) -> Unit = { event ->
        ...
        when (event) {
            is CastEvent.Play -> {
                playerManager?.playUrl(event.url, (event.startPosition * 1000).toLong())
            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (videoUrl != null && (videoUrl.startsWith("http://") || videoUrl.startsWith("https://"))) {
            Log.d(TAG, "Starting playback: $videoUrl")
            playerManager?.playUrl(videoUrl, (startPosition * 1000).toLong())
        } else {
